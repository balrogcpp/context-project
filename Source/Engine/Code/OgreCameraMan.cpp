...
void CameraMan::frameRendered(float timeSinceLastFrame) {
    if (mStyle == CS_FREELOOK)
    {
...
void CameraMan::OnKeyEvent(SDL_Scancode key, bool pressed) {
    if (mStyle == CS_FREELOOK)
    {
        if (pressed) {
            if (key == SDL_SCANCODE_W || key == SDLK_UP) mGoingForward = true;
            else if (key == SDL_SCANCODE_S || key == SDLK_DOWN) mGoingBack = true;
            else if (key == SDL_SCANCODE_A || key == SDLK_LEFT) mGoingLeft = true;
            else if (key == SDL_SCANCODE_D || key == SDLK_RIGHT) mGoingRight = true;
            else if (key == SDLK_PAGEUP) mGoingUp = true;
            else if (key == SDLK_PAGEDOWN) mGoingDown = true;
            else if (key == SDLK_LSHIFT) mFastMove = true;
        } else {
            if (key == SDL_SCANCODE_W || key == SDLK_UP) mGoingForward = false;
            else if (key == SDL_SCANCODE_S || key == SDLK_DOWN) mGoingBack = false;
            else if (key == SDL_SCANCODE_A || key == SDLK_LEFT) mGoingLeft = false;
            else if (key == SDL_SCANCODE_D || key == SDLK_RIGHT) mGoingRight = false;
...
void CameraMan::OnMouseMotion(int xrel, int yrel) {
    if (mStyle == CS_ORBIT)
    {
        Ogre::Real dist = getDistToTarget();
...
void CameraMan::OnMouseWheel(int x, int y) {
    if (mStyle == CS_ORBIT && y != 0)
    {
        Ogre::Real dist = (mCamera->getPosition() - mTarget->_getDerivedPosition()).length();
        mCamera->translate(Ogre::Vector3(0, 0, -y * 0.08f * dist), Ogre::Node::TS_LOCAL);
    }
}

//bool CameraMan::mousePressed(const MouseButtonEvent &evt) {
void CameraMan::OnMouseButton(int button, bool pressed) {
    if (mStyle == CS_ORBIT)
    {
        if (pressed) {
            if (button == 0) mOrbiting = true;
            else if (button == 2) mMoving = true;
        } else {
            if (button == 0) mOrbiting = false;
            else if (button == 2) mMoving = false;
        }
    }
}
