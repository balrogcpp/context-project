...
void ImpostorPage::build() {
  if (m_mapImpostorBatches.empty()) return;

  // Calculate the average Y value of all the added entities
  if (m_nAveCount > 0) {
    m_vecCenter.y = m_vecCenter.y / static_cast<Ogre::Real>(m_nAveCount);
  } else {
    m_vecCenter.y = 0.0;
  }
...
void ImpostorPage::regenerate(Entity *ent) {
  ...
  if (tex != nullptr) tex->regenerate();
}

void ImpostorPage::regenerateAll() { ImpostorTexture::regenerateAll(); }

//-------------------------------------------------------------------------------------

unsigned long ImpostorBatch::GUID = 0;

ImpostorBatch::ImpostorBatch(ImpostorPage *group, Entity *entity) : m_pTexture(nullptr) {
...
ImpostorTexture::ImpostorTexture(ImpostorPage *group, Entity *entity) : loader(0) {
...
ImpostorTexture::~ImpostorTexture() {
  // Delete textures
  if (TextureManager::getSingletonPtr()) TextureManager::getSingleton().remove(texture);
  texture.reset();
...
void ImpostorTexture::renderTextures(bool force) {
  ...
  // if we're not using a file image we need to set up a resource loader, so that the texture is
  // regenerated if it's ever unloaded (such as switching between fullscreen and the desktop in
  // win32)
  loader = std::shared_ptr<ImpostorTextureResourceLoader>(new ImpostorTextureResourceLoader(*this));
#endif
  ...
  if (!renderTexture) {
    renderTexture = TextureManager::getSingleton().createManual(
        getUniqueID("ImpostorTexture"), "Impostors", TEX_TYPE_2D, textureSize * IMPOSTOR_YAW_ANGLES,
        textureSize * IMPOSTOR_PITCH_ANGLES, 0, PF_BYTE_RGBA, TU_RENDERTARGET, loader.get());
  }
  ...
  if (TextureManager::getSingletonPtr()) TextureManager::getSingleton().remove(renderTexture);
#endif
}
