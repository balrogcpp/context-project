...
vec3 godotAtmosphere(const highp vec3 eye_dir, const highp vec3 light_dir) {
    ...
    float mie_phase = henyey_greenstein_godot(cos_theta, mie_eccentricity);
    ...
    color = 2.0 / (1.0 + exp(-1.0 * color)) - 1.0;
    return color;
}
...
vec3 ogreAtmosphere(const highp vec3 eye_dir, const highp vec3 p_sunDir) {
    ...
    float LdotV = max(dot(atmoCameraDir, p_sunDir), 0.0);
    ...
    float LdotV360 = dot(atmoCameraDir, p_sunDir) * 0.5 + 0.5;
    ...
    vec3 sharedTerms = skyColourGradient * skyAbsorption;
...
vec3 hwAtmosphere(const highp vec3 V, const highp vec3 N)
{
...
vec3 atmosphere(const highp vec3 V, const highp vec3 N)
{
    return hwAtmosphere(V, N);
}
