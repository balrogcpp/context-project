...
vec3 vignette(const vec3 color, const highp vec2 uv, const vec4 vignette, const vec3 vignetteColor) {
    ...
    distance.x *= aspect;
    distance = pow(clamp(distance, 0.0, 1.0), vec2(radius, radius));

    float amount = pow(clamp(1.0 - dot(distance, distance), 0.0, 1.0), feather * 5.0);
    return color * mix(vignetteColor, vec3(1.0, 1.0, 1.0), amount);
}
