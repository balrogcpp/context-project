...
def _write_b2o_ver(fd):
    ...
    fd.write(bytes('import PBR from pbr.material\n\n', 'utf-8'))
...
#                     r.append( prog.data )
    ...
    def generate_texture_unit(self, key, texture):
        ...
        if not key in ("base_color_texture", "normalmap_texture", "roughness_texture", "metallic_texture", "emission_color_texture"):
        ...
        elif key == 'normalmap_texture':
            tex = 'Normal'

        elif key == 'roughness_texture':
            tex = 'ORM'

        elif key == 'emission_color_texture':
            tex = 'Emission'
        ...
        if key == 'normalmap_texture':
            self.copy_texture(image, 'none')
        else:
            self.copy_texture(image, 'bc5')

        with self.w.iword('texture_unit ' + tex).embed():
            self.w.iword('texture').word(filename).nl()

            exmode = texture.extension
            if exmode in TEXTURE_ADDRESS_MODE and exmode != 'REPEAT':
    ...
    def copy_texture(self, image, compress=True):
...
def gather_metallic_roughness_texture(mat_wrapper):
    ...
    for input_name in ['Roughness', 'Metallic']:
        ...
        if separate_node.type not in ['SEPARATE_COLOR', 'SEPRGB']:
            logger.warn("Connected node '%s' has type '%s', not 'SEPARATE_COLOR'" % (separate_node.name, separate_node.type))
...
def gather_metallic_roughness_factor(mat_wrapper):
    """
    For a given material, retrieve the corresponding metallic roughness texture according to glTF2 guidelines.
    (https://docs.blender.org/manual/en/2.80/addons/io_scene_gltf2.html#metallic-and-roughness)
    :param blender_material: a blender material for which to get the metallic roughness texture
