# include guard
if (_thirdparty_included)
    return()
endif (_thirdparty_included)
set(_thirdparty_included true)
...
if ((CMAKE_CXX_COMPILER_ID MATCHES "Clang" OR CMAKE_CXX_COMPILER_ID STREQUAL "GNU") AND NOT MSVC)
    set(DEPS_EXE_LINKER_FLAGS "${DEPS_EXE_LINKER_FLAGS} -s")
endif ()
...
macro(check_stamp_option TARGET OPTION)
    set(STAMP_FILE_DOWNLOAD ${DEPS_SOURCE_LOCATION}/${TARGET}_download)
    set(UPDATE_PLACEHOLDER ${CMAKE_COMMAND} -E true)
    set(PATCH_PLACEHOLDER ${CMAKE_COMMAND} -E true)
    set(CREATE_DOWNLOAD_STAMP ${CMAKE_COMMAND} -E touch ${STAMP_FILE_DOWNLOAD})
    ...
    if ((EXISTS ${STAMP_FILE_DOWNLOAD} AND NOT IS_DIRECTORY ${STAMP_FILE_DOWNLOAD}) OR OPTION)
        set(DOWNLOAD_PLACEHOLDER ${CMAKE_COMMAND} -E true)
        set(DOWNLOAD_PLACEHOLDER ${CMAKE_COMMAND} -E true)
    else ()
        unset(DOWNLOAD_PLACEHOLDER)
...
check_stamp_patch(Target_SDL2 sdl2-2.30.3.patch)
...
check_stamp_patch(Target_assimp assimp-5.4.0.patch)
...
check_stamp(Target_OgreProcedural)
...
check_stamp(Target_ParticleUniverse)
externalproject_add(Target_ParticleUniverse
        EXCLUDE_FROM_ALL true
        BUILD_ALWAYS false
        BUILD_IN_SOURCE false
        DEPENDS Target_OGRE
        PREFIX ${DEPS_PREFIX_LOCATION}
        DOWNLOAD_DIR ${DEPS_SOURCE_LOCATION}
        SOURCE_DIR ${DEPS_SOURCE_LOCATION}/Target_ParticleUniverse
        GIT_REPOSITORY https://github.com/OGRECave/particleuniverse.git
        GIT_TAG fa7467748e0e2402a7f700125a0af391d733aa9d
        GIT_SHALLOW false
        GIT_PROGRESS false
        DOWNLOAD_COMMAND ${DOWNLOAD_PLACEHOLDER}
        UPDATE_COMMAND ${UPDATE_PLACEHOLDER}
...
if (Protobuf_PROTOC_EXECUTABLE)
    ...
    if (PROTOC_VERSION)
        if (${PROTOC_VERSION} STREQUAL "libprotoc 25.3")
            set(PROTOC_FOUND 1)
            message(STATUS "Found protoc: ${Protobuf_PROTOC_EXECUTABLE} (${PROTOC_VERSION})")
        else ()
            set(PROTOC_FOUND 0)
            message(STATUS "Found protoc: ${Protobuf_PROTOC_EXECUTABLE} (${PROTOC_VERSION}). Required version is 25.3")
        endif ()
    else ()
        set(PROTOC_FOUND 0)
        message(STATUS "protoc not found")
    endif ()
else ()
    set(PROTOC_FOUND 0)
    message(STATUS "protoc not found")
...
if (NOT PROTOC_FOUND)
    if (CMAKE_CROSSCOMPILING)
        set(BUILD_PROTOC_CROSS ON)
    else ()
        set(BUILD_PROTOC_NATIVE ON)
    endif ()
endif ()
...
if (BUILD_PROTOC_CROSS)
    set(PROTOBUF_DEPENDS Target_protoc)
endif ()
check_stamp_option(Target_protobuf BUILD_PROTOC_CROSS)
externalproject_add(Target_protobuf
        ...
        -Dprotobuf_BUILD_EXAMPLES=OFF
        -Dprotobuf_BUILD_PROTOBUF_BINARIES=ON
        -Dprotobuf_BUILD_PROTOC_BINARIES=${BUILD_PROTOC_NATIVE}
        -Dprotobuf_BUILD_SHARED_LIBS=OFF
        -Dprotobuf_MSVC_STATIC_RUNTIME=OFF
        -Dprotobuf_WITH_ZLIB=OFF
        -Dprotobuf_DISABLE_RTTI=OFF
...
check_stamp_patch(Target_GameNetworkingSockets gns-de03d74.patch)
externalproject_add(Target_GameNetworkingSockets
        ...
        GIT_TAG de03d74226eb3b9a299e05f5fff93965d3dce2d9
        GIT_SHALLOW false
        GIT_PROGRESS false
