...
macro(FlatZipDirectory curdir destination)
    get_filename_component(PARENT_DIR ${destination} DIRECTORY ABSOLUTE)
    if (PARENT_DIR)
    ...
    if (ZIP_EXE AND ZIP_VERSION VERSION_GREATER_EQUAL 3.0)
        message(STATUS "Using zip ${ZIP_VERSION} to archive assets")
        execute_process(COMMAND ${CMAKE_COMMAND} -E chdir ${curdir} ${ZIP_EXE} -FSrq -D -X ${destination} ${filelist})
    elseif (TAR_EXE AND TAR_VERSION MATCHES zlib)
        message(STATUS "Zip not found, using tar to archive assets")
        execute_process(COMMAND ${CMAKE_COMMAND} -E chdir ${curdir} ${TAR_EXE} caf ${destination} ${filelist})
    else ()
        message(STATUS "Zip or bsdtar not found, using cmake command-line to archive assets")
        execute_process(COMMAND ${CMAKE_COMMAND} -E chdir ${curdir} ${CMAKE_COMMAND} -E tar cf ${destination} --format=zip ${filelist})
    endif ()
    if (STRIP_EXE)
