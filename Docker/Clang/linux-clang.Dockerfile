...
ARG GCC_HOME=/opt/gcc

RUN apt-get update \
    && apt-get install --no-install-recommends -y gcc-9 g++-9 zlib1g-dev libgmp-dev libmpfr-dev libmpc-dev libssl-dev libisl-dev libisl10 libmpc2 \
    && apt-get clean \
...
RUN update-alternatives --install /usr/bin/gcc gcc ${GCC_HOME}/bin/x86_64-linux-gnu-gcc 11200 \
...
RUN update-alternatives --install /usr/local/bin/python3 python3 ${PYTHON3_HOME}/bin/python${PYTHON3_SHORT_VERSION} 3990
