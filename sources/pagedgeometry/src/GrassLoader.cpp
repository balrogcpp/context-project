	...
	if (shaderNeedsUpdate){
		...
		if (caps->hasCapability(RSC_VERTEX_PROGRAM) && geom->getShadersEnabled())
		{
			...
			if (!tmpMat)
			{
				...
				HighLevelGpuProgramPtr vertexShader = Ogre::static_pointer_cast<HighLevelGpuProgram>(HighLevelGpuProgramManager::getSingleton().getByName(vsName, Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME));
				if (!vertexShader)
				{
...
void GrassPage::removeEntities()
{
	...
	for (i = nodeList.begin(); i != nodeList.end(); ++i)
	{
		...
		for(int j = 0; j < numObjs; j++)
		{
			...
			if(!ent) continue;
			// remove the mesh
			MeshManager::getSingleton().remove(ent->getMesh()->getName(), Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME);
			// then the entity
