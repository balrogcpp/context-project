...
void TreeLoader2D::addTree(Entity *entity, const Vector3 &position, Degree yaw, Real scale, void *userData) {
  ...
  tree.rotation =
      static_cast<uint8>(Ogre::Real(0.708) * yaw.valueDegrees());                       // 0.708 ~= 255 / 360
    ...
    for (int tileZ = minPageZ; tileZ <= maxPageZ; ++tileZ) {
      for (int tileX = minPageX; tileX <= maxPageX; ++tileX) {
        ...
        while (i < treeList.size()) {
          //Get tree distance
          Ogre::Real
              distX = (gridBounds.left + (tileX * pageSize) + ((Real) treeList[i].xPos / 65535) * pageSize) - pos.x;
          Ogre::Real
              distZ = (gridBounds.top + (tileZ * pageSize) + ((Real) treeList[i].zPos / 65535) * pageSize) - pos.z;
...
void TreeIterator2D::moveNext() {
  ...
  if (!hasMore)
    OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR,
                "Cannot read past end of TreeIterator list",
