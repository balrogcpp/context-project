...
void PagedGeometry::setPageSize(Real size) {
  if (!managerList.empty())
    OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR,
                "PagedGeometry::setPageSize() cannot be called after detail levels have been added. Call removeDetailLevels() first.",
...
void PagedGeometry::setInfinite() {
  if (!managerList.empty())
    OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR,
                "PagedGeometry::setInfinite() cannot be called after detail levels have been added. Call removeDetailLevels() first.",
...
void PagedGeometry::setBounds(TBounds bounds) {
  if (!managerList.empty())
    OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR,
                "PagedGeometry::setBounds() cannot be called after detail levels have been added. Call removeDetailLevels() first.",
...
void PagedGeometry::reloadGeometryPage(Vector3 point) {
  if (!pageLoader)
    return;
...
//-----------------------------------------------------------------------------
///
float PagedGeometry::getCustomParam(const Ogre::String &entity,
...
void GeometryPageManager::update(unsigned long deltaTime,
                                 Vector3 &camPos,
                                 Vector3 &camSpeed,
                                 bool &enableCache,
      ...
      if (distSq + pageLengthSq >= nearDistSq && distSq - pageLengthSq < farTransDistSq) {
        ...
        if (fadeEnabled && distSq + pageLengthSq >= farDistSq) {
          ...
          fadeNear = prevManager->farDist + (prevManager->farTransDist - prevManager->farDist)
              * 0.5f;    //This causes geometry to fade out faster than it fades in, avoiding a state where a transition appears semitransparent
...
void GeometryPage::addEntityToBoundingBox(Ogre::Entity *ent,
