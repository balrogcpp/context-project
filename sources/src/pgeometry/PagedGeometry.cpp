...
void PagedGeometry::setTempDir(Ogre::String dir) { tempdir = dir; }

void PagedGeometry::setPageLoader(PageLoader *loader) { pageLoader = loader; }
...
void PagedGeometry::setCamera(Camera *cam) {
  if (cam == nullptr) {
    ...
    if (sceneMgr == nullptr) sceneMgr = sceneCam->getSceneManager();
...
void PagedGeometry::setCoordinateSystem(Vector3 up, Vector3 right) {
  ...
  if (rootNode) rootNode->setOrientation(coordinateSystemQuat);
}
#endif
...
Vector3 PagedGeometry::_convertToLocal(const Vector3 &globalVec) const {
  assert(getSceneNode());
  // Convert from the given global position to the local coordinate system of PagedGeometry's root
  // scene node.
  return (getSceneNode()->getOrientation().Inverse() * globalVec);
}
#else
// Default coordinate system - no conversion
Vector3 PagedGeometry::_convertToLocal(const Vector3 &globalVec) const { return globalVec; }
#endif

void PagedGeometry::setPageSize(Real size) {
  if (!managerList.empty())
    OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR,
                "PagedGeometry::setPageSize() cannot be called after detail levels have been "
                "added. Call removeDetailLevels() first.",
                "PagedGeometry::setPageSize()");
...
void PagedGeometry::setInfinite() {
  if (!managerList.empty())
    OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR,
                "PagedGeometry::setInfinite() cannot be called after detail levels have been "
                "added. Call removeDetailLevels() first.",
...
void PagedGeometry::setBounds(TBounds bounds) {
  if (!managerList.empty())
    OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR,
                "PagedGeometry::setBounds() cannot be called after detail levels have been added. "
...
//-----------------------------------------------------------------------------
///
float PagedGeometry::getCustomParam(const Ogre::String &entity, const Ogre::String &paramName,
                                    float defaultParamValue) const {
...
GeometryPageManager::GeometryPageManager(PagedGeometry *mainGeom) : mainGeom(mainGeom) {
...
GeometryPageManager::~GeometryPageManager() {
  ...
  for (int x = 0; x < geomGridX; ++x)
    for (int z = 0; z < geomGridZ; ++z) delete _getGridPage(x, z);
...
void GeometryPageManager::update(unsigned long deltaTime, Vector3 &camPos, Vector3 &camSpeed, bool &enableCache,
  ...
  if (scrollBuffer) {
    ...
    if (x1 < 0)
      shiftX = x1;
    else if (x2 >= geomGridX - 1)
      shiftX = x2 - (geomGridX - 1);
    if (z1 < 0)
      shiftZ = z1;
    else if (z2 >= geomGridZ - 1)
      shiftZ = z2 - (geomGridZ - 1);
...
void GeometryPageManager::reloadGeometryPages(const Vector3 &center, Real radius) {
  ...
  if (x1 < 0)
    x1 = 0;
  else if (x1 > geomGridX - 1)
    x1 = geomGridX - 1;
  if (z1 < 0)
    z1 = 0;
  else if (z1 > geomGridZ - 1)
    z1 = geomGridZ - 1;
...
void GeometryPageManager::_scrollGridPages(int shiftX, int shiftZ) {
  ...
  if (shiftX > geomGridX || shiftX < -geomGridX || shiftZ > geomGridZ || shiftZ < -geomGridZ) {
    // If so, just reload all the tiles (reloading is accomplished by unloading - loading is
    // automatic)
    for (int x = 0; x < geomGridX; ++x) {
      for (int z = 0; z < geomGridZ; ++z) {
        GeometryPage *page = _getGridPage(x, z);
    ...
    if (shiftX > 0) {
      for (int z = 0; z < geomGridZ; ++z) {
        ...
        for (int x = 0; x < shiftedMidpoint; ++x) _setGridPage(x, z, _getGridPage(x + shiftX, z));
    ...
    if (shiftZ > 0) {
      for (int x = 0; x < geomGridX; ++x) {
        ...
        for (int z = 0; z < shiftedMidpoint; ++z) _setGridPage(x, z, _getGridPage(x, z + shiftZ));
        // Rotate temporary pages around to other side of grid
        for (int z = 0; z < shiftZ; ++z) {
...
const AxisAlignedBox &GeometryPage::getBoundingBox() { return _trueBounds; }
