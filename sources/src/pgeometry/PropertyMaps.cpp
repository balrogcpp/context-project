  ...
  if (channel == CHANNEL_COLOR) {
    ...
    switch (channel) {
      ...
      case CHANNEL_ALPHA:
        ...
        break;
      default:
        OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS, "Invalid channel", "GrassLayer::setDensityMap()");
        break;
    }
...
Ogre::Real DensityMap::_getDensityAt_Unfiltered(Ogre::Real x, Ogre::Real z, const TRect<Real> &mapBounds) {
  ...
  if (x < mapBounds.left || x >= mapBounds.right || z < mapBounds.top || z >= mapBounds.bottom) return 0.f;
...
Ogre::Real DensityMap::_getDensityAt_Bilinear(Ogre::Real x, Ogre::Real z, const TRect<Ogre::Real> &mapBounds) {
  ...
  if (xIndex < 0 || zIndex < 0 || xIndex >= mapWidth - 1 || zIndex >= mapHeight - 1) return 0.0f;
...
ColorMap::ColorMap(TexturePtr map, MapChannel channel) {
  ...
  switch (format) {
    ...
    case VET_UBYTE4_NORM:
      ...
      if (buff->getFormat() == PF_L8) channel = CHANNEL_RED;
      break;
    default:
  ...
  if (channel == CHANNEL_COLOR) {
    ...
    switch (channel) {
      ...
      case CHANNEL_ALPHA:
        ...
        break;
      default:
        OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS, "Invalid channel", "ColorMap::ColorMap()");
        break;
    }
...
uint32 ColorMap::_getColorAt_Bilinear(Ogre::Real x, Ogre::Real z, const TRect<Real> &mapBounds) {
  ...
  uint32 xIndex = (uint32)xIndexFloat;
  uint32 zIndex = (uint32)zIndexFloat;
  if (xIndex < 0 || zIndex < 0 || xIndex > mapWidth - 1 || zIndex > mapHeight - 1) return 0xFFFFFFFF;
