...
#include "pcheader.h"
#include "pgeometry/TreeLoader3D.h"
...
TreeLoader3D::TreeLoader3D(PagedGeometry *geom, const TBounds &bounds) {
  ...
  // Make sure the bounds are aligned with PagedGeometry's grid, so the TreeLoader's grid tiles will have a 1:1
  // relationship
  actualBounds = bounds;
  gridBounds = bounds;
  gridBounds.left =
      pageSize * Math::Floor((gridBounds.left - geom->getBounds().left) / pageSize) + geom->getBounds().left;
  gridBounds.top = pageSize * Math::Floor((gridBounds.top - geom->getBounds().top) / pageSize) + geom->getBounds().top;
  gridBounds.right =
      pageSize * Math::Ceil((gridBounds.right - geom->getBounds().left) / pageSize) + geom->getBounds().left;
  gridBounds.bottom =
      pageSize * Math::Ceil((gridBounds.bottom - geom->getBounds().top) / pageSize) + geom->getBounds().top;
...
void TreeLoader3D::addTree(Entity *entity, const Ogre::Vector3 &position, Degree yaw, Real scale, void *userData) {
  ...
  if (pos.x < actualBounds.left - smallVal || pos.x > actualBounds.right + smallVal ||
      pos.z < actualBounds.top - smallVal || pos.z > actualBounds.bottom + smallVal)
...
TreeLoader3D::deleteTrees(const Vector3 &position, Real radius, Entity *type) {
  ...
  if (minPageX < 0)
    minPageX = 0;
  else if (minPageX >= pageGridX)
    minPageX = pageGridX - 1;
  if (minPageZ < 0)
    minPageZ = 0;
  else if (minPageZ >= pageGridZ)
    minPageZ = pageGridZ - 1;
  if (maxPageX < 0)
    maxPageX = 0;
  else if (maxPageX >= pageGridX)
    maxPageX = pageGridX - 1;
  if (maxPageZ < 0)
    maxPageZ = 0;
  else if (maxPageZ >= pageGridZ)
    maxPageZ = pageGridZ - 1;
    ...
    for (int tileZ = minPageZ; tileZ <= maxPageZ; ++tileZ) {
      for (int tileX = minPageX; tileX <= maxPageX; ++tileX) {
        ...
        while (i < treeList.size()) {
          // Get tree distance
          Ogre::Real distX =
              (gridBounds.left + (tileX * pageSize) + ((Real)treeList[i].xPos / 65535) * pageSize) - pos.x;
          Ogre::Real distZ =
              (gridBounds.top + (tileZ * pageSize) + ((Real)treeList[i].zPos / 65535) * pageSize) - pos.z;
...
TreeLoader3D::deleteTrees(TBounds area, Ogre::Entity *type) {
  ...
  if (area.left < actualBounds.left)
    area.left = actualBounds.left;
  else if (area.left > actualBounds.right)
    area.left = actualBounds.right;
  if (area.top < actualBounds.top)
    area.top = actualBounds.top;
  else if (area.top > actualBounds.bottom)
    area.top = actualBounds.bottom;
  if (area.right < actualBounds.left)
    area.right = actualBounds.left;
  else if (area.right > actualBounds.right)
    area.right = actualBounds.right;
  if (area.bottom < actualBounds.top)
    area.bottom = actualBounds.top;
  else if (area.bottom > actualBounds.bottom)
    area.bottom = actualBounds.bottom;
...
TreeIterator3D TreeLoader3D::getTrees() { return TreeIterator3D(this); }
...
TreeIterator3D::TreeIterator3D(TreeLoader3D *trees) {
  ...
  if (currentTree == currentTreeList->end()) moveNext();
...
void TreeIterator3D::moveNext() {
  // Out of bounds check
  if (!hasMore)
    OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR, "Cannot read past end of TreeIterator list",
  ...
  if (currentTree != currentTreeList->end()) ++currentTree;
