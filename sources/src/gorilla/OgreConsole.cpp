...
void OgreConsole::init(Gorilla::Screen *screen) {
  ...
  mDecoration->background_gradient(Gorilla::Gradient_NorthSouth, Gorilla::rgb(128, 128, 128, 128),
...
void OgreConsole::OnKeyDown(SDL_Keycode arg) {
  if (arg == SDLK_BACKQUOTE) setVisible(!isVisible());
  ...
  if (arg == SDLK_RETURN) {
    ...
    if (!prompt.empty()) prompt.pop_back();
    mUpdatePrompt = true;
  } else if (arg == SDLK_PAGEUP) {
    if (mStartline > 0) mStartline--;
    mUpdateConsole = true;
  } else if (arg == SDLK_PAGEDOWN) {
    if (mStartline < lines.size()) mUpdateConsole = true;
...
bool OgreConsole::frameStarted(const Ogre::FrameEvent &evt) {
  if (mUpdateConsole) updateConsole();

  if (mUpdatePrompt) updatePrompt();
...
void OgreConsole::updateConsole() {
  ...
  if (mStartline > lines.size()) mStartline = lines.size();
...
void OgreConsole::addCommand(const Ogre::String &command, OgreConsoleFunctionPtr func) { commands[command] = func; }

void OgreConsole::removeCommand(const Ogre::String &command) { commands.erase(commands.find(command)); }

#if OGRE_VERSION_MINOR < 8 && OGRE_VERSION_MAJOR < 2
void OgreConsole::messageLogged(const Ogre::String &message, Ogre::LogMessageLevel lml, bool maskDebug,
