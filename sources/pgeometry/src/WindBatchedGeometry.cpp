...
//-----------------------------------------------------------------------------
///
void WindBatchedGeometry::addEntity(Entity *ent,
...
void WindBatchedGeometry::WindSubBatch::build() {
  ...
  vertDecl->addElement(numVertBuffs - 1,
                       vertDecl->getVertexSize(0),
                       VET_FLOAT4,
  ...
  for (size_t iMesh = 0, meshCnt = m_queueMesh.size(); iMesh < meshCnt; ++iMesh) {
    ...
    float factorX = static_cast<WindBatchedGeometry *>(m_pParentGeom)->mGeom->getCustomParam(ent->getName(),
                                                                                             c_windFactorX,
                                                                                             0.f);    // amplitude in X
    float factorY = static_cast<WindBatchedGeometry *>(m_pParentGeom)->mGeom->getCustomParam(ent->getName(),
                                                                                             c_windFactorY,
                                                                                             0.f);    // amplitude in Y
