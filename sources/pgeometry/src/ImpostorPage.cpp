...
//-----------------------------------------------------------------------------
///
void ImpostorPage::addEntity(Entity *ent,
...
void ImpostorBatch::setBillboardOrigin(BillboardOrigin origin) {
  ...
  if (bbset->getBillboardOrigin() == BBO_CENTER)
    ...
    entityBBCenter = Vector3(m_pTexture->entityCenter.x,
                             m_pTexture->entityCenter.y - m_pTexture->entityRadius,
...
void ImpostorTexture::renderTextures(bool force) {
  ...
  if (!renderTexture) {
    renderTexture = TextureManager::getSingleton().createManual(getUniqueID("ImpostorTexture"),
                                                                "Impostors",
                                                                TEX_TYPE_2D,
                                                                textureSize * IMPOSTOR_YAW_ANGLES,
                                                                textureSize * IMPOSTOR_PITCH_ANGLES,
                                                                0,
                                                                PF_BYTE_RGBA,
                                                                TU_RENDERTARGET,
                                                                loader.get());
  ...
  Ogre::SceneManager::SpecialCaseRenderQueueMode
      OldSpecialCaseRenderQueueMode = sceneMgr->getSpecialCaseRenderQueueMode();
