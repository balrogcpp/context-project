...
DensityMap *DensityMap::load(const String &fileName, MapChannel channel) {
  //Load image
  TexturePtr map =
      TextureManager::getSingleton().load(fileName, ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, TEX_TYPE_2D, 0);
...
DensityMap::DensityMap(TexturePtr map, MapChannel channel) {
  ...
  if (channel == CHANNEL_COLOR) {
    ...
    switch (channel) {
      case CHANNEL_RED: channelOffset = 3;
        break;
      case CHANNEL_GREEN: channelOffset = 2;
        break;
      case CHANNEL_BLUE: channelOffset = 1;
        break;
      case CHANNEL_ALPHA: channelOffset = 0;
        break;
      default: OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS, "Invalid channel", "GrassLayer::setDensityMap()");
        break;
    }
...
ColorMap::ColorMap(TexturePtr map, MapChannel channel) {
  ...
  switch (format) {
    ...
    case VET_COLOUR_ABGR:
      ...
      if (buff->getFormat() == PF_L8)
        channel = CHANNEL_RED;
      break;
    default:OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS, "Unknown RenderSystem color format", "GrassLayer::setColorMap()");
      break;
  }
  ...
  if (channel == CHANNEL_COLOR) {
    ...
    switch (channel) {
      ...
      case CHANNEL_ALPHA: channelOffset = 0;
        break;
      default: OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS, "Invalid channel", "ColorMap::ColorMap()");
        break;
    }
