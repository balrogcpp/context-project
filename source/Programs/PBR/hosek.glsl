...
vec3 spectral_radiance(float cos_theta, float gamma, float sun_zenith)
{
    vec3 XYZ;
    for (int i = 0; i < 3; ++i) {
        float coeffs[9];
...
{
    return spectral_radiance(cos_theta, gamma, sun_zenith) * mean_spectral_radiance(sun_zenith);
}
