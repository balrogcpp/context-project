...
float Downscale4x4(sampler2D tex, const vec2 uv, const vec2 tsize)
{
    ...
    float D = luminance(texture2D(tex, uv + tsize * vec2(1.0, 1.0)).rgb) + 0.0001;
    float E = luminance(texture2D(tex, uv + tsize * vec2(2.0, 0.0)).rgb) + 0.0001;
    float F = luminance(texture2D(tex, uv + tsize * vec2(3.0, 0.0)).rgb) + 0.0001;
    float G = luminance(texture2D(tex, uv + tsize * vec2(2.0, 1.0)).rgb) + 0.0001;
    float H = luminance(texture2D(tex, uv + tsize * vec2(3.0, 1.0)).rgb) + 0.0001;
    float I = luminance(texture2D(tex, uv + tsize * vec2(0.0, 2.0)).rgb) + 0.0001;
    float J = luminance(texture2D(tex, uv + tsize * vec2(1.0, 2.0)).rgb) + 0.0001;
    float K = luminance(texture2D(tex, uv + tsize * vec2(0.0, 3.0)).rgb) + 0.0001;
    float L = luminance(texture2D(tex, uv + tsize * vec2(1.0, 3.0)).rgb) + 0.0001;
    float M = luminance(texture2D(tex, uv + tsize * vec2(2.0, 2.0)).rgb) + 0.0001;
    float N = luminance(texture2D(tex, uv + tsize * vec2(3.0, 2.0)).rgb) + 0.0001;
...
void main()
{
    ...
    vec2 ratio = (TexSize0.xy * ViewportSize.zw) * 0.25;
    float lum = Downscale4x4(RT, vUV0, TexSize0.zw * ratio);
