...
void main()
{
    ...
    if (PixelWasRenderedThisFrame()) {
        FragColor.rgb = SafeHDR(texture(MRT, vUV0).rgb);
        return;
    }
    ...
    vec3 A = texture(MRT, vUV0 + texelSize0 * vec2(0.0, 1.0)).rgb;
    vec3 B = texture(MRT, vUV0 + texelSize0 * vec2(0.0, -1.0)).rgb;
    vec3 C = texture(MRT, vUV0 + texelSize0 * vec2(-1.0, 0.0)).rgb;
    vec3 D = texture(MRT, vUV0 + texelSize0 * vec2(1.0, 0.0)).rgb;

    // pixel was render prev frame, reconstruct it's position in prev frame
    vec2 velocity = texture(VelocityTex, vUV0).xy;
    float depth1 = texture(DepthTex, vUV0).x;
