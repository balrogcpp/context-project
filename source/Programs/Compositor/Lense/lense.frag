...
vec3 GhostFeatures(sampler2D tex, const vec2 uv, int counter, float radius)
{
...
vec3 HaloFeatures(sampler2D tex, const vec2 uv, int counter, float radius)
{
    vec2 nuv = vec2(1.0, 1.0) - uv;
...
void main()
{
    ...
    //color += GhostFeatures(RT, vUV0, FeaturesCount, ChromaticRadius) * 0.5;
    color += HaloFeatures(RT, vUV0, FeaturesCount, ChromaticRadius);
    FragColor.rgb = color;
}
