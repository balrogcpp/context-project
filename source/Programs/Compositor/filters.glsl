...
//----------------------------------------------------------------------------------------------------------------------
vec3 Linear(const sampler2D sampler, const vec2 uv, const vec2 tsize)
{
...
//----------------------------------------------------------------------------------------------------------------------
vec3 GaussV(const sampler2D sampler, const vec2 uv, const vec2 tsize)
{
  vec3 A = texture2D(sampler, uv).rgb;
  vec3 B = texture2D(sampler, uv + tsize * vec2(0.0, 1.3846153846)).rgb;
  vec3 C = texture2D(sampler, uv - tsize * vec2(0.0, 1.3846153846)).rgb;
  vec3 D = texture2D(sampler, uv + tsize * vec2(0.0, 3.2307692308)).rgb;
  vec3 E = texture2D(sampler, uv - tsize * vec2(0.0, 3.2307692308)).rgb;
...
//----------------------------------------------------------------------------------------------------------------------
vec3 GaussH(const sampler2D sampler, const vec2 uv, const vec2 tsize)
{
  vec3 A = texture2D(sampler, uv).rgb;
  vec3 B = texture2D(sampler, uv + tsize * vec2(1.3846153846, 0.0)).rgb;
  vec3 C = texture2D(sampler, uv - tsize * vec2(1.3846153846, 0.0)).rgb;
  vec3 D = texture2D(sampler, uv + tsize * vec2(3.2307692308, 0.0)).rgb;
  vec3 E = texture2D(sampler, uv - tsize * vec2(3.2307692308, 0.0)).rgb;
...
//----------------------------------------------------------------------------------------------------------------------
vec3 Upscale2x2(const sampler2D sampler, const vec2 uv, const vec2 tsize)
{
  vec3 A = texture2D(sampler, uv + (tsize * vec2(-0.5, -0.5))).rgb;
  vec3 B = texture2D(sampler, uv + (tsize * vec2(-0.5, 0.5))).rgb;
  vec3 C = texture2D(sampler, uv + (tsize * vec2(-0.5, 0.5))).rgb;
...
//----------------------------------------------------------------------------------------------------------------------
vec3 FXAA(const sampler2D sampler, const vec2 uv, const vec2 tsize, const float strength)
{
  const float reducemul = 0.125; // 1/8
  const float reducemin = 0.0078125; // 1/128
  const vec3 gray = vec3(0.299, 0.587, 0.114);

  vec3 basecol = texture2D(sampler, uv).rgb;
  vec3 baseNW = texture2D(sampler, uv - tsize).rgb;
  vec3 baseNE = texture2D(sampler, uv + vec2(tsize.x, -tsize.y)).rgb;
  vec3 baseSW = texture2D(sampler, uv + vec2(-tsize.x, tsize.y)).rgb;
  vec3 baseSE = texture2D(sampler, uv + tsize).rgb;
  ...
  vec3 resultA = 0.5 * (texture2D(sampler, uv + dir * -0.166667).rgb + texture2D(sampler, uv + dir * 0.166667).rgb);
  vec3 resultB = resultA * 0.5 + 0.25 * (texture2D(sampler, uv + dir * -0.5).rgb + texture2D(sampler, uv + dir * 0.5).rgb);
