...
// Note: p_data, p_min and p_max are _pointers_ to a memory address holding the data. For a slider, they are all required.
// Read code of e.g. SliderFloat(), SliderInt() etc. or examples in 'Demo->Widgets->Data Types' to understand how to use this function directly.
bool SliderScalarB(const char *label, ImGuiDataType data_type, void *p_data, const void *p_min, const void *p_max, const char *format,
  ...
  if (!temp_input_is_active) {
    ...
    if (input_requested_by_tabbing || clicked || g.NavActivateId == id || g.NavActivateInputId == id) {
      ...
      if (temp_input_allowed && (input_requested_by_tabbing || (clicked && g.IO.KeyCtrl) || g.NavActivateInputId == id)) temp_input_is_active = true;
    }
  }
  ...
  ImRect grab_bb;
  float percent = 0;
  bool value_changed = SliderBehaviorB(slider_bb, id, data_type, p_data, p_min, p_max, format, ImGuiSliderFlags_None, &grab_bb, &percent);
...
bool SliderInt(const char *label, int *v, int v_min, int v_max, const char *format, ImGuiSliderFlags flags) {
  return SliderScalarB(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
