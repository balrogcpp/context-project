...
WindBatchedGeometry::WindBatchedGeometry(SceneManager *mgr, SceneNode *rootSceneNode, const PagedGeometry *pagedGeom)
    : BatchedGeometry(mgr, rootSceneNode), mGeom(pagedGeom) {
  // empty
}

//-----------------------------------------------------------------------------
///
void WindBatchedGeometry::addEntity(Entity *ent, const Vector3 &position, const Quaternion &orientation,
...
void WindBatchedGeometry::WindSubBatch::build() {
  ...
  unsigned short texCoordCount = 0;
  {
    ...
    while (it != iend) {
      if ((*it).getSemantic() == VES_TEXTURE_COORDINATES) ++texCoordCount;
      ++it;
    }
  }
  ...
  vertDecl->addElement(numVertBuffs - 1, vertDecl->getVertexSize(0), VET_FLOAT4, VES_TEXTURE_COORDINATES,
                       texCoordCount);
  vertDecl->addElement(numVertBuffs - 1, vertDecl->getVertexSize(0), VET_FLOAT4, VES_TEXTURE_COORDINATES,
                       texCoordCount + 1);
  ...
  for (size_t iMesh = 0, meshCnt = m_queueMesh.size(); iMesh < meshCnt; ++iMesh) {
    ...
    float factorX = static_cast<WindBatchedGeometry *>(m_pParentGeom)
                        ->mGeom->getCustomParam(ent->getName(), c_windFactorX,
                                                0.f);  // amplitude in X
    float factorY = static_cast<WindBatchedGeometry *>(m_pParentGeom)
                        ->mGeom->getCustomParam(ent->getName(), c_windFactorY,
                                                0.f);  // amplitude in Y
    ...
    for (Ogre::ushort i = 0; i < destBinds->getBufferCount(); ++i) {
      if (i < sourceBinds->getBufferCount()) {
        ...
        // vector to stock the original y value of every vertex because batchCenter doesn't take consider the height of
        // the ground
